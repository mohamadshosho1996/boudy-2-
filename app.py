...
@keyframes heartPulse {
    ...
    85% {
        transform: rotate(-45deg) scale(1) skew(20deg);
        clip-path: polygon(0 0, 50% 0, 50% 100%, 0 100%);
        margin-left: -40px;
        opacity: 0.8;
    }
    100% {
        transform: rotate(-45deg) scale(1) translateX(-80px);
        opacity: 0;
    }
}
...
# ==================== إعدادات الاتصال الآمن بـ Google Sheets ====================
@st.cache_resource
  ...
  if spreadsheet:
    ...
    if "المشورة الاسرية للحامل" not in existing_sheets:
      ws = spreadsheet.add_worksheet(
          title="المشورة الاسرية للحامل", rows=100, cols=len(PREGNANT_COLUMNS)
      )
      ws.append_row(PREGNANT_COLUMNS)
    if "سجل المشورة للاطفال" not in existing_sheets:
      ws = spreadsheet.add_worksheet(
          title="سجل المشورة للاطفال", rows=100, cols=len(CHILD_COLUMNS)
      )
...
PREGNANT_COLUMNS = [
    ...
    (
        "حركة الجنين / معرفة جنس الجنين/ تمييز الأصوات من قبل"
        " الجنين"
    ),
        ...
        for s in [
          ...
          try:
            ...
            if id_column in df.columns:
              match = df[
                  df[id_column].astype(str).str.strip() == clean_id
              ]
              if not match.empty:
...
# عرض أنيميشن القلب والسهم وكلمة شيماء إذا تم تفعيلها
...
if menu == "الصفحة الرئيسية":
  st.markdown(
      "<h1>✨ مرحباً بكِ في نظام المشورة الأسرية الشامل (سحابي) ✨</h1>",
      unsafe_allow_html=True,
  )
  st.write(
      "تم ربط البرنامج بنجاح مع Google Sheets على جوجل درايف ليتم حفظ ومزامنة"
      " بيانات الحوامل والأطفال لحظياً."
  )
...
# ==================== 2. سجل الحوامل ====================
  ...
  for col_name in PREGNANT_COLUMNS:
    ...
    if col_name == "نوع الولادة":
      ...
      if "p_birth_nat" not in st.session_state:
        st.session_state.p_birth_nat = False
      if "p_birth_ces" not in st.session_state:
        st.session_state.p_birth_ces = False
      if "p_birth_none" not in st.session_state:
      ...
      with c_opt1:
        st.checkbox("طبيعى", key="p_birth_nat", on_change=p_update_nat)
      with c_opt2:
        st.checkbox("قيصرى", key="p_birth_ces", on_change=p_update_ces)
      with c_opt3:
        st.checkbox("لا يوجد", key="p_birth_none", on_change=p_update_none)

      selected_birth = ""
      if st.session_state.p_birth_nat:
        selected_birth = "طبيعى"
      elif st.session_state.p_birth_ces:
        selected_birth = "قيصرى"
      elif st.session_state.p_birth_none:
      ...
      if col_name == "الرقم القومى":
        ...
        form_data[col_name] = st.text_input(
            f"{col_name} [محسوب تلقائياً من الرقم القومي]", key=f"p_{col_name}"
        )
  ...
  if st.button("💾 حفظ بيانات الحامل", use_container_width=True):
    ...
    if save_new_row(
        "المشورة الاسرية للحامل", final_form_data, PREGNANT_COLUMNS
    ):
...
# ==================== 3. سجل الأطفال ====================
  ...
  for col_name in CHILD_COLUMNS:
    ...
    if col_name == "نوع الولادة":
      ...
      if "c_birth_nat" not in st.session_state:
        st.session_state.c_birth_nat = False
      if "c_birth_ces" not in st.session_state:
        st.session_state.c_birth_ces = False
      if "c_birth_none" not in st.session_state:
      ...
      c_opt1, c_opt2, c_opt3 = st.columns(3)
      with c_opt1:
        st.checkbox("طبيعى", key="c_birth_nat", on_change=c_update_nat)
      with c_opt2:
        st.checkbox("قيصرى", key="c_birth_ces", on_change=c_update_ces)
      with c_opt3:
        st.checkbox("لا يوجد", key="c_birth_none", on_change=c_update_none)

      selected_birth = ""
      if st.session_state.c_birth_nat:
        selected_birth = "طبيعى"
      elif st.session_state.c_birth_ces:
        selected_birth = "قيصرى"
      elif st.session_state.c_birth_none:
        selected_birth = "لا يوجد"
      ...
      if col_name in ["الرقم القومى للام", "الرقم القومى للاب"]:
        ...
        st.text_input(
            f"{col_name} [محسوب تلقائياً]", key=f"c_{col_name}"
        )
        ...
        try:
          ...
          head_birth = float(
              st.session_state.get("c_مقاس راس الطفل عند الولادة", "35.0")
              or 35.0
          )
          if age_m <= 12:
          ...
          if current_visit in VISIT_SCHEDULE_OPTIONS:
            ...
            if idx + 1 < len(VISIT_SCHEDULE_OPTIONS):
              ...
              if "شهر" in next_visit_name:
                m_num = int(
                    "".join(filter(lambda x: x.isdigit(), next_visit_name))
                    or 1
                )
  ...
  if st.button("💾 حفظ بيانات الطفل", use_container_width=True):
    ...
    for col in CHILD_COLUMNS:
      if col == "تاريخ التسجيل":
        ...
        if (
            col == "سبب دخول الحضانة"
            and st.session_state.get("c_دخول الحضانة", "") != "تم"
        ):
          val = ""
...
# ==================== 4. استعراض البيانات والداشبورد ====================
  ...
  if spreadsheet:
    ...
    date_col = (
        "تاريخ الزيارة"
        if "تاريخ الزيارة" in df_view.columns
        else "تاريخ التسجيل"
    )
      ...
      try:
        ...
        with col_user_filter:
          if "اسم المستخدم" in df_view.columns:
            users_list = ["الكل"] + list(
                df_view["اسم المستخدم"].dropna().unique()
            )
            selected_user_filter = st.selectbox(
                "👩‍⚕️ تصفية حسب الطبيبة:",
                users_list,
                key="filter_user_selectbox",
            )
          else:
    ...
    if (
        selected_user_filter != "الكل"
        and "اسم المستخدم" in filtered_df.columns
    ):
    ...
    if sheet_to_show == "المشورة الاسرية للحامل":
      ...
      if (
          "رضاعة طبيعية مطلقة" in filtered_df.columns
          and not filtered_df.empty
      ):
    ...
    if st.session_state.role == "admin":
      ...
      st.error(
          ...
          " البيانات نهائياً من جوجل شيت السحابي."
      )
            ...
            try:
              ...
              if id_col_target in current_df.columns:
                ...
                current_df = current_df[
                    current_df[id_col_target].astype(str).str.strip()
                    != cleaned_del_id
                ]
                ...
                if deleted_count > 0:
                  ...
                  st.success(
                      f"تم حذف عدد ({deleted_count}) سجل بالرقم القومي بنجاح من"
                      " الشيت السحابي! ✨"
                  )
                  st.rerun()
                else:
                  st.warning(
                      "لم يتم العثور على سجل بهذا الرقم القومي في الشيت الحالي."
                  )
              else:
                st.error(
    ...
    st.warning(
        "تعذر الاتصال بملف جوجل شيت 'FamilyCareDB' على درايف. تأكد من صحة"
        " الاسم والصلاحيات."
    )
...
# ==================== 5. إدارة المستخدمين ====================
  ...
  for k, v in DEFAULT_USERS.items():
    st.write(
        f"- **{v['name']}** | اسم المستخدم: `{k}` | الصلاحية: `{v['role']}`"
    )
